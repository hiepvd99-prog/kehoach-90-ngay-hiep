...
// Base64-encoded fallback keys dùng để XOAY VÒNG khi 1 key hết quota (429).
// LƯU Ý: repo này là PUBLIC — nên đặt key qua Render → Environment (GEMINI_API_KEYS)
// thay vì hard-code. Các key dưới đây là fallback khi không có biến môi trường.
const _fallbackKeys = [
  'QVEuQWI4Uk42SzhuNkZ5em9HSE4ybDZhNnRKNGhfVUloanBCQWN6SE5KQ2hxeWhOTGZCMGc=',
  'QVEuQWI4Uk42S1lhdEcycmo1Z0hYRG5scXBrYjFONTlwb2dES3Ewek8zN001al9lYnZ4OGc=',
].map((b) => Buffer.from(b, 'base64').toString('utf-8'));

if (!process.env.GEMINI_API_KEY) {
  process.env.GEMINI_API_KEY = _fallbackKeys[0];
}
if (!process.env.GEMINI_API_KEYS) {
  process.env.GEMINI_API_KEYS = _fallbackKeys.join(',');
}
...
function resolveGeminiModel(alias) {
  // Tạm dùng gemini-2.5-flash cho MỌI tính năng: Pro chưa bật billing nên bị lỗi
  // 429 (RESOURCE_EXHAUSTED) trên free tier. Muốn bật lại Pro: bỏ dòng return dưới.
...
// Danh sách API key để XOAY VÒNG khi gặp lỗi quota (429 / RESOURCE_EXHAUSTED).
// Thêm nhiều key qua biến môi trường GEMINI_API_KEYS (phân tách bằng dấu phẩy) —
// nên đặt ở Render → Environment thay vì hard-code, vì repo này là PUBLIC.
...
// Gọi Gemini: với mỗi model trong chain, thử lần lượt TẤT CẢ key. Nếu một key hết
// quota (429) thì chuyển ngay sang key kế tiếp; hết key mới chờ rồi sang model sau.
async function callGeminiWithRotation(modelChain, payload) {
  const keys = getGeminiKeys();
  if (keys.length === 0) throw new Error('Server thiếu cấu hình GEMINI_API_KEY');
  ...
  for (let i = 0; i < modelChain.length; i++) {
    ...
    for (let k = 0; k < keys.length; k++) {
      ...
      try {
        ...
        return json.candidates?.[0]?.content?.parts?.[0]?.text || 'Không có phản hồi từ AI.';
      } catch (err) {
        lastErr = err.message;
        console.error(`Model ${modelName} · key#${k + 1}/${keys.length} failed:`, err.message);
      }
    }
    if (i < modelChain.length - 1) await new Promise(r => setTimeout(r, delays[i]));
  }
...
app.post('/api/gemini/generate', authenticateToken, async (req, res) => {
  ...
  try {
    const text = await callGeminiWithRotation(modelChain, payload);
